...
const Transactions = () => {
  ...
  const handleExportPDF = async () => {
    try {
      ...
      // If we are using server-side pagination and have more pages, we might need to fetch all.
      // However, fetching *all* transactions might be heavy. 
      // For now, let's try to export what's available in `allTransactions` if it contains everything (client-side filtering case),
      // or fetch all if we are server-side paginated.
      
      if (!hasClientSideFilters() && count > transactions.length) {
         // Server-side pagination is active and we don't have all records.
         // We need to fetch all records matching current filters.
         const params = { ...filters };
         delete params.page;
         delete params.limit; // backend should return all or a large limit if limit is missing/0, dependent on API implementation. 
         // Safest is to request a large limit if backend supports it, or loop. 
         // Let's try setting a high limit.
         params.limit = 10000; 
         ...
         exportData = response.data.results || [];
          ...
          // If client side filters are on, `filteredTransactions` contains all filtered results
          // `sortedData` is derived from `transactions` which is a slice of `filteredTransactions`
          // So we should use `filteredTransactions` but we need to apply the current sort to it.
          // Since `useTableSort` is a hook, we can't just call it here. 
          // We can manually sort `filteredTransactions` using the current sort config.
      ...
      doc.autoTable({
        columns: columns,
        body: tableData,
        startY: 40, // Start table below title and metadata
